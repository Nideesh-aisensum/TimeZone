...
async function startHeartbeat() {
    ...
    async function sendHeartbeat() {
        try {
            await cloudPool.query(`
                INSERT INTO kiosk_heartbeats (kiosk_id, place, status, last_heartbeat, server_started_at)
                ...
                ON CONFLICT (kiosk_id) DO UPDATE SET
                    ...
                    last_heartbeat = NOW(),
                    server_started_at = $3
            `, [kioskId, place, serverStartedAt]);
        } catch (e) {
            // Silent fail — no network is expected sometimes
        }
    }

    // Send immediately
    await sendHeartbeat();
    console.log(`💓 Heartbeat started for kiosk ${kioskId} (every 2 min)`);
